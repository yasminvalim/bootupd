...
Boot0003* test";
    ...
    fn test_package_mode_copy_to_boot_discovery() -> Result<()> {
        ...
        std::fs::create_dir_all(efi_path.join("shim/1.0/EFI/fedora"))?;
        std::fs::create_dir_all(efi_path.join("grub2/1.0/EFI/fedora"))?;
        std::fs::write(
            efi_path.join("shim/1.0/EFI/fedora/shimx64.efi"),
            b"shim content",
        )?;
        std::fs::write(
            efi_path.join("grub2/1.0/EFI/fedora/grubx64.efi"),
            b"grub content",
        )?;
        ...
        // Verify paths are correct and versions were discovered
    ...
    fn test_package_mode_shim_installation() -> Result<()> {
        ...
        let efi_path = tpath.join("usr/lib/efi");
        let shim_path = efi_path.join("shim/1.0/EFI/fedora");
        ...
        assert_eq!(efi_comps[0].version, "1.0");
        ...
        // Source: /usr/lib/efi/shim/{version}/EFI/fedora/shimx64.efi
